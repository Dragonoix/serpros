...
const HomePageModel = require('homePage/models/homePage.model');
...
const homePageRepository = {
    ...
    getAll: async (req) => {
        try {
            ...
            let aggregate = HomePageModel.aggregate([
                { $match: conditions },
                sortOperator
            ],{ collation: { locale: "en",caseFirst:"upper" } });
            let options = { page: req.body.page, limit: req.body.length };
            let allHomePage = await HomePageModel.aggregatePaginate(aggregate, options);
    ...
    getById: async (id) => {
        try {
            let datas = await HomePageModel.findById(id).lean().exec();
            if (!datas) {
                return null;
            }
            return datas;
    ...
    getByField: async (params) => {
        try {
            let datas = await HomePageModel.findOne(params).exec();
            if (!datas) {
                return null;
            }
            return datas;
    ...
    getDistinctDocument: async (field, params) => {
        try {
            let datas = await HomePageModel.distinct(field, params);
            if (!datas) {
                return null;
            }
            return datas;
    ...
    getAllByField: async (params) => {
        try {
            let datas = await HomePageModel.find(params).exec();
            if (!datas) {
                return null;
            }
            return datas;
    ...
    getHomePageCount: async (params) => {
        try {
            let count = await HomePageModel.countDocuments(params);
            if (!count) {
                return null;
            }
            return count;
    ...
    save: async (data) => {
        try {
            let datas = await HomePageModel.create(data);
            if (!datas) {
                return null;
            }
            return datas;
    ...
    delete: async (id) => {
        try {
            let datas = await HomePageModel.findById(id);
            if (datas) {
                let deleted = await HomePageModel.deleteOne({ _id: id }).exec();
                if (!deleted) {
                    return null;
                }
                return deleted;
    ...
    updateById: async (data, id) => {
        try {
            let datas = await HomePageModel.findByIdAndUpdate(id, data, { new: true });
            if (!datas) {
                return null;
            }
            return datas;
