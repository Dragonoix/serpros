$(function () {
    ...
    if (dtUserTable.length) {
        dtUserTable.on('processing.dt', function () {
            ...
            columns: [
                ...
                { data: '' },
                { data: 'stripe_transactionId' },
                { data: 'payer.full_name' },
                { data: 'payee.full_name' },
                { data: 'paymentStatus' },
                { data: '' },
            ],
            columnDefs: [
                {
                    ...
                    render: function (data, type, full, meta) {
                        return moment(full['trans_date']).format('MM-DD-YYYY (h:mm:ss a)')
                    }
                },
                {
                    targets: 2,
                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                      return (
                        '<a href="' + window.location.protocol + '//' + window.location.host + '/transaction/detail/' + full["_id"] + '" style="color:blue;">View</a>'
                      );
                    }
                  }
            ],
    ...
    $(document).on('click', '.add-new', function () {
        window.location.href = `${window.location.protocol}//${window.location.host}/transaction/create`;
    });
