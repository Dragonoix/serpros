$(function () {
    ...
    if (dtUserTable.length) {
        dtUserTable.on('processing.dt', function () {
            ...
            serverSide: true,
            ajax: {
                url: `${window.location.protocol}//${window.location.host}/review/getall`,
                method: 'post',
            ...
            columns: [
                ...
                { data: '' },
                { data: 'reviewFor.full_name' },
                { data: 'reviewBy.full_name' },
                { data: 'avgRating' },
                { data: 'review_date' },
                { data: '' },
            ],
            columnDefs: [
                ...
                },
                {
                    ...
                    sortable: true,
                    render: function (data, type, full, meta) {
                        // let avg = parseFloat(full['workmanship']['rating']) + parseFloat(full['cost']['rating']) + 
                        // parseFloat(full['schedule']['rating']) + parseFloat(full['communication']['rating']);
                        return full['avgRating']
                    }
                },
                {
                    targets: 4,
                    ...
                    searchable: true,
                    render: function (data, type, full, meta) {
                        return moment(full['review_date']).format('MM-DD-YYYY')
                    }
                },
                {
                    // Actions
                    ...
                    render: function (data, type, full, meta) {
                        return (
                            '<a href="' + window.location.protocol + '//' + window.location.host + '/review/detail/' + full["_id"] + '" style="color:blue;">View</a>'
                        );
                    }
                }
            ],
    ...
    $(document).on('click', '.add-new', function () {
        window.location.href = `${window.location.protocol}//${window.location.host}/review/create`;
    });
    ...
    $(document).on('click', '.categoryStatusUpadte', function () {
        ...
        swal.fire({
            ...
            if (result.value) {
                window.location.href = `${window.location.protocol}//${window.location.host}/review/status-change/${elemID}`;
            }
        });
    });
    ...
    $(document).on('click', '.deleteUser', function () {
        ...
        swal.fire({
            ...
            if (result.value) {
                window.location.href = `${location.protocol}//${window.location.host}/review/delete/${elemID}`;
            }
        });
    });
});
